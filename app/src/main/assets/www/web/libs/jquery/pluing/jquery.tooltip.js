...
(function($) {
	...
	plugin = $.fn.Tooltip = function(settings)	{
		...
		if( !helper ) {
			...
			helper = $('<div id="tooltip"><h3></h3><p class="tooltip-body"></p><p class="url"></p></div>')
			// hide it at first
			.hide()
	...
	save = function() {
		...
		source.attr('title','');
		this.tSettings.oldTitle=oldTitle;

		if(settings.showBody && title) {
	...
	*/
	update = function(event)	{
		...
		if(pos('X')+15+helper.width()<screen.width-50 && pos('Y')+15+helper.height()<screen.height-180){
		...
		}else{if(pos('X')+15+helper.width()>screen.width-50 && pos('Y')+15+helper.height()>screen.height-180)
		{


			helper.css({
				left: pos('X')-helper.width()-25  + 'px',
				top: pos('Y')-helper.height()-15  + 'px'
			});
		}else
		{  if(pos('X')+15+helper.width()<screen.width-50 && pos('Y')+15+helper.height()>screen.height-180)
		{
		...
		{
			if(event.pageX-helper.width()<0)
			{
				helper.css({
					top: pos('Y') + 15 + 'px',
					left: pos('X')-helper.width()+450+'px'
				});
			}else
			{
				helper.css({
